...
// DefaultOverloadConfig 默认过载保护配置
...
func (op *OverloadProtector) AllowNewRequest() bool {
	...
	if currentQPS >= warningThreshold {
		...
		time.Sleep(time.Millisecond) // 1ms延迟，避免突发流量
	}

	return true
}

// AllowUpstreamRequest 检查是否允许上游请求
...
// IsConnectionOverloaded 检查连接是否过载
